...
@Component({
  selector: 'app-npsiquiatrica',
  templateUrl: './npsiquiatrica.component.html',
  styleUrls: ['./npsiquiatrica.component.css']
})
export class NpsiquiatricaComponent implements OnInit {

	npsiquiatricaForm: FormGroup;
	escolaridad: any = [];
    etnia: any = [];
    ocupacion: any = [];
    discapacidad: any = [];
	...
	ngAfterViewInit(): void {
  	...
  	initForm() {
        this.npsiquiatricaForm = this.formBuilder.group({
            DATOSHISTORIA: [''],
            DATOSPACIENTE: [''],
            ID_HISTORIA: [''],
            DATOS: [''],
    ...
    Guardar() {
        ...
        this.npsiquiatricaForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.npsiquiatricaForm.get('DATOSHISTORIA').setValue(datos);
    	this.npsiquiatricaForm.get('DATOSPACIENTE').setValue(pac);
        this.npsiquiatricaForm.get('DATOS').setValue(datos1);
        this.npsiquiatricaForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.npsiquiatricaForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.npsiquiatricaForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.npsiquiatricaForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.npsiquiatricaForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    Page1(datos) {
        var html = '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;">EVOLUCION / NOTA:</div>' +
                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>' +
                '</div>' +
                '<div class="col-md-12">'+datos[4]['VALOR']+'</div>';
        return html;
    }
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>NOTA EVOLUCION CONTROL</h3></div>';
        return html;
    }
    ...
    CrearImagen(pdf, div, terminar) {
        ...
        html2canvas(div).then(canvas => {
            ...
            if(terminar == true) {
                ...
                that.tipo_id.emit(0);
                /*pdf.autoPrint();
                pdf.save('NOTAEVOLUCIONCONTROL.pdf');*/
    ...
    clearAll() {
        this.npsiquiatricaForm.get('DATOSHISTORIA').setValue('');
        this.npsiquiatricaForm.get('DATOSPACIENTE').setValue('');
        this.npsiquiatricaForm.get('DATOS').setValue('');
        this.npsiquiatricaForm.get('PACIENTE').setValue('');
        this.npsiquiatricaForm.get('PRESTADOR').setValue('');
        this.npsiquiatricaForm.get('ID_HISTORIA').setValue(0);
        this.npsiquiatricaForm.get('ID_CITA').setValue(0);
