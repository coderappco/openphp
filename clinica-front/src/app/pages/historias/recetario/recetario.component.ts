...
export class RecetarioComponent implements OnInit {
	...
	ngAfterViewInit(): void {
  	...
  	initForm() {
        this.recetarioForm = this.formBuilder.group({
            DATOSHISTORIA: [''],
            DATOSPACIENTE: [''],
            ID_HISTORIA: [''],
            DATOS: [''],
    ...
    Guardar() {
        ...
        this.recetarioForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.recetarioForm.get('DATOSHISTORIA').setValue(datos);
    	this.recetarioForm.get('DATOSPACIENTE').setValue(pac);
        this.recetarioForm.get('DATOS').setValue(datos1);
        this.recetarioForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.recetarioForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.recetarioForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.recetarioForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.recetarioForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    Page1(datos) {
        var html = '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;">DETALLES RECETA:</div>' +
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>RECETARIO</h3></div>';
        return html;
    }
    ...
    clearAll() {
        this.recetarioForm.get('DATOSHISTORIA').setValue('');
        this.recetarioForm.get('DATOSPACIENTE').setValue('');
        this.recetarioForm.get('DATOS').setValue('');
        this.recetarioForm.get('PACIENTE').setValue('');
        this.recetarioForm.get('PRESTADOR').setValue('');
        this.recetarioForm.get('ID_HISTORIA').setValue(0);
        this.recetarioForm.get('ID_CITA').setValue(0);
