...
export class UrgenciasComponent implements OnInit {
	...
	ngAfterViewInit(): void {
  	...
  	initForm() {
        this.urgenciaForm = this.formBuilder.group({
            DATOSHISTORIA: [''],
            DATOSPACIENTE: [''],
            ID_HISTORIA: [''],
            DATOS: [''],
    ...
    Guardar() {
        ...
        this.urgenciaForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.urgenciaForm.get('DATOSHISTORIA').setValue(datos);
    	this.urgenciaForm.get('DATOSPACIENTE').setValue(pac);
        this.urgenciaForm.get('DATOS').setValue(datos1);
        this.urgenciaForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.urgenciaForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.urgenciaForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.urgenciaForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.urgenciaForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    clearAll() {
        this.urgenciaForm.get('DATOSHISTORIA').setValue('');
        this.urgenciaForm.get('DATOSPACIENTE').setValue('');
        this.urgenciaForm.get('DATOS').setValue('');
        this.urgenciaForm.get('PACIENTE').setValue('');
        this.urgenciaForm.get('PRESTADOR').setValue('');
        this.urgenciaForm.get('ID_HISTORIA').setValue(0);
        this.urgenciaForm.get('ID_CITA').setValue(0);
    ...
    Page1(datos, page = null) {
        var motivo = datos[115]['VALOR'] != null ? datos[115]['VALOR'] : "NO REFIERE";
    ...
    Page2(datos, page = null) {
    	var imc = (datos[65]['VALOR'] != 0 && datos[64]['VALOR'] != 0) ? datos[64]['VALOR']/datos[65]['VALOR'] : 0;
        var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;padding-right: 15px;padding-left: 15px;flex: 0 0 99%;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
    ...
    Page3(datos, da) {
        var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;padding-right: 15px;padding-left: 15px;flex: 0 0 99%;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(da[datos[118]['CAMPO']] != '' ? da[datos[118]['CAMPO']] : "NO REFIERE")+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;padding-right: 15px;padding-left: 15px;flex: 0 0 99%;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>HISTORIA CLINICA DE URGENCIAS</h3></div>';
        return html;
    }
    ...
    CrearImagen(pdf, div, terminar) {
        ...
        html2canvas(div).then(canvas => {
            ...
            if(terminar == true) {
                ...
                that.tipo_id.emit(0);
                /*pdf.autoPrint();
                pdf.save('HISTORIACLINICAURGENCIAS.pdf');*/
