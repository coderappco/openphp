...
@Component({
  	selector: 'app-eodontologia',
  	templateUrl: './eodontologia.component.html',
  	styleUrls: ['./eodontologia.component.css']
})
export class EodontologiaComponent implements OnInit {
    ...
    GenerarPdf(id) {
        ...
        this.historiasService.getHistoriaPacientes(id)
            .subscribe(data => {
                ...
                html = header + this.Page2(da) + this.Prestador(da.usuario);
    ...
    Page1(datos, da, page = null) {
        ...
        let motivo = datos[15]['VALOR'] != null ? datos[15]['VALOR'] : "";
        var imc = (datos[6]['VALOR'] != 0 && datos[5]['VALOR'] != 0) ? datos[6]['VALOR']/datos[5]['VALOR'] : 0;
        ...
        var html = '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(datos[16]['VALOR'] != null ? datos[16]['VALOR'] : '')+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;">EVOLUCION:</div>' +
                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(datos[17]['VALOR'] != null ? datos[17]['VALOR'] : '')+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;">DIAGNOSTICO:</div>' +
                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>';
                for(var j in diag) {
                    html += '<div class="col-md-12" style="text-transform: uppercase !important;">'+diag[j]['diagnostico']['COD_DIAGNOSTICO']+" "+diag[j]['diagnostico']['NOM_DIAGNOSTICO']+'</div>';
                }
                html += '</div>' +
                '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>HISTORIA CLINICA - EVOLUCION ODONTOLOGIA</h3></div>';
        return html;
    }
    ...
    CrearImagen(pdf, div, terminar) {
        ...
        html2canvas(div).then(canvas => {                           
            ...
            if(terminar == true) {
                ...
                that.tipo_id.emit(0);
                /*pdf.autoPrint();
                pdf.save('HISTORIACLINICAEODONTOLOGICA.pdf');*/
