...
@Component({
  	selector: 'app-evolucionmed',
  	templateUrl: './evolucionmed.component.html',
  	styleUrls: ['./evolucionmed.component.css']
})
export class EvolucionmedComponent implements OnInit {
    ...
    Guardar() {
        ...
        this.evolucionForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.evolucionForm.get('DATOSHISTORIA').setValue(datos);
    	this.evolucionForm.get('DATOSPACIENTE').setValue(pac);
        this.evolucionForm.get('DATOS').setValue(datos1);
        this.evolucionForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.evolucionForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.evolucionForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.evolucionForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.evolucionForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    Page1(datos, da) {
        ...
        var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">EVOLUCION:</div>' +
                ...
                '<div class="col-md-12">'+(datos[4]['VALOR'] != null ? datos[4]['VALOR'] : "NO REFIERE")+'</div>'+
                '</div>' +
                ...
                html += '</div>'+
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">TIPO DE DIAGNOSTICO:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-9 mt-2" style="text-transform: uppercase !important;">'+datos[5]['VALOR']+'</div>' +
                '</div>';
        return html;
    }
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>HISTORIA CLINICA EVOLUCION MEDICA</h3></div>';
        return html;
    }
    ...
    clearAll() {
        this.evolucionForm.get('DATOSHISTORIA').setValue('');
        this.evolucionForm.get('DATOSPACIENTE').setValue('');
        this.evolucionForm.get('DATOS').setValue('');
        this.evolucionForm.get('PACIENTE').setValue('');
        this.evolucionForm.get('PRESTADOR').setValue('');
        this.evolucionForm.get('ID_HISTORIA').setValue(0);
        this.evolucionForm.get('ID_CITA').setValue(0);
