...
export class VihComponent implements OnInit {
	...
	ngAfterViewInit(): void {
  	...
  	initForm() {
        this.vihForm = this.formBuilder.group({
            DATOSHISTORIA: [''],
            DATOSPACIENTE: [''],
            ID_HISTORIA: [''],
            DATOS: [''],
    ...
    Guardar() {
        ...
        this.vihForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.vihForm.get('DATOSHISTORIA').setValue(datos);
    	this.vihForm.get('DATOSPACIENTE').setValue(pac);
        this.vihForm.get('DATOS').setValue(datos1);
        this.vihForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.vihForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.vihForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.vihForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.vihForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    Page1(datos) {
        var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;padding-right: 15px;padding-left: 15px;flex: 0 0 99%;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12">'+(datos[1]['VALOR'] != null ? datos[1]['VALOR'] : "NO REFIERE")+'</div>' +
                '</div></br></br>' +
                '<div class="col-md-5"><div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div></div>' +
                '<div class="col-md-2"></div>' +
                '<div class="col-md-5"><div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div></div>' +
                '<div class="col-md-5 mb-5">Firma de quien recibe la asesoria:</div>' +
                '<div class="col-md-2"></div>' +
                '<div class="col-md-5">cc:</div>' +
                '<div class="col-md-5"><div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div></div>' +
                '<div class="col-md-2"></div>' +
                '<div class="col-md-5"><div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div></div>' +
                '<div class="col-md-5 mb-5">Firma del compañero:</div>' +
                '<div class="col-md-2"></div>' +
                '<div class="col-md-5">cc:</div>' +
                '<div class="col-md-5"><div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div></div>' +
                '<div class="col-md-2"></div>' +
                '<div class="col-md-5"><div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div></div>' +
                '<div class="col-md-5 mb-5">Firma y sello del asesor:</div>' +
                '<div class="col-md-2"></div>' +
                '<div class="col-md-5">cc:</div>';
        return html;
    }
    ...
    clearAll() {
        this.vihForm.get('DATOSHISTORIA').setValue('');
        this.vihForm.get('DATOSPACIENTE').setValue('');
        this.vihForm.get('DATOS').setValue('');
        this.vihForm.get('PACIENTE').setValue('');
        this.vihForm.get('PRESTADOR').setValue('');
        this.vihForm.get('ID_HISTORIA').setValue(0);
        this.vihForm.get('ID_CITA').setValue(0);
