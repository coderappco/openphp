...
@Component({
  	selector: 'app-concentinf',
  	templateUrl: './concentinf.component.html',
  	styleUrls: ['./concentinf.component.css']
})
export class ConcentinfComponent implements OnInit {
    ...
    Guardar() {
        ...
        this.concentForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.concentForm.get('DATOSHISTORIA').setValue(datos);
    	this.concentForm.get('DATOSPACIENTE').setValue(pac);
        this.concentForm.get('DATOS').setValue(datos1);
        this.concentForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.concentForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.concentForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.concentForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.concentForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    Page1(datos) {
        var html = '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12 text-center" style="background-color: #f3f3f3;font-weight: bolder;">CONSENTIMIENTO PARA RECIBIR ANESTESIA:</div>' +
                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>' +
                '</div>' +
                '<div class="col-md-12" style="margin-bottom: 6rem !important;"><p>El Doctor: </p>'+ 
                'Me ha explicado la naturaleza y propósitos del procedimiento, también me ha informado de las ventajas, complicaciones, molestias y riesgos que puedan producirse.</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">______________________________________________________</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;"></div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">FIRMA DEL PACIENTE O PERSONA RESPONSABLE:</div>'+
                '<div class="col-md-6" style="text-transform: uppercase !important;"></div>'+
                '<div class="col-md-6 mb-4">CC No:</div>'+
                '<div class="col-md-12 mb-5">Certifico que he explicado la naturaleza, propósito, ventajas, riesgos y alternativas del procedimiento especial, y he contestado todas las preguntas. Considero que el (la) paciente, pariente o tutor comprende completamente lo explicado</div>';
        return html;
    }
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>CONSENTIMIENTO INFORMADO</h3></div>';
        return html;
    }
    ...
    CrearImagen(pdf, div, terminar) {
        ...
        html2canvas(div).then(canvas => {
            ...
            if(terminar == true) {
                ...
                that.tipo_id.emit(0);
                /*pdf.autoPrint();
                pdf.save('CONSENTIMIENTOINFORMADO.pdf');*/
    ...
    clearAll() {
        this.concentForm.get('DATOSHISTORIA').setValue('');
        this.concentForm.get('DATOSPACIENTE').setValue('');
        this.concentForm.get('DATOS').setValue('');
        this.concentForm.get('PACIENTE').setValue('');
        this.concentForm.get('PRESTADOR').setValue('');
        this.concentForm.get('ID_HISTORIA').setValue(0);
        this.concentForm.get('ID_CITA').setValue(0);
