...
@Component({
  	selector: 'app-ordenesmedicas',
  	templateUrl: './ordenesmedicas.component.html',
  	styleUrls: ['./ordenesmedicas.component.css']
})
export class OrdenesmedicasComponent implements OnInit {
    ...
    @Input() servi: any;
  	...
  	ngOnChanges(changes: SimpleChanges) {
        ...
        const se: SimpleChange = changes.servi;
        ...
        if(tipo)
            if((tipo.currentValue == 23 || tipo.currentValue == 34) && pr) {
                if(pr.currentValue == true)
                    this.GenerarPdf(this.historia_id);
            }
        if(se)
            if(se.currentValue.length > 0) {
                $('#addexamen').html('');
	            this.servicio = se.currentValue;
	            var html = "";
	            for(var i in this.servicio){
	                let html = $('#addexamen').html();
	                html = html + '<div class="col-md-5 mb-1 mt-1">'+this.servicio[i]['servicio']+'</div>'+
								'<div class="col-md-2 mb-1 mt-1 text-center">'+this.servicio[i]['cantidad']+'</div>'+
								'<div class="col-md-3 mb-1 mt-1">'+this.servicio[i]['descripcion']+'</div>'+
								'<div class="col-md-2 mb-1 mt-2"><a href="#" pos="'+i+'" title="Eliminar servicio" data-toggle="tooltip" class="boton_examen"><i class="zmdi zmdi-delete"></i></a></div>';
	                $('#addexamen').html(html);
	            }
            }
            else
	...
	ngAfterViewInit(): void {
        ...
        this.itemsService.getItemsServ()
            .subscribe(data => this.items = data);
        $("body").on("click", ".boton_examen", function(a) {
            a.preventDefault();
            let arr = [];
            let pos = 0;
            for(var i in that.servicio) {
                if(i != $(this).attr('pos')) {
                    arr[pos] = that.servicio[i];
                    pos++;
                }
            }
            $('#addexamen').html('');
            ...
            for(var i in that.servicio){
                let html = $('#addexamen').html();
                html = html + '<div class="col-md-5 mb-1 mt-1">'+arr[i]['servicio']+'</div>'+
                              '<div class="col-md-2 mb-1 mt-1 text-center">'+arr[i]['cantidad']+'</div>'+
                              '<div class="col-md-3 mb-1 mt-1 text-center">'+arr[i]['descripcion']+'</div>'+
                              '<div class="col-md-2 mb-1 mt-2"><a href="#" pos="'+i+'" title="Eliminar servicio" data-toggle="tooltip" class="boton_examen"><i class="zmdi zmdi-delete"></i></a></div>';
                $('#addexamen').html(html);
            }
        });
    ...
    Guardar() {
    	...
    	if(this.servicio.length == 0) {
    		alert("Por favor, debe asignar los servicios al paciente.");
    		return false;
    	}
        ...
        this.ordenesForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.ordenesForm.get('DATOSHISTORIA').setValue(datos);
    	this.ordenesForm.get('DATOSPACIENTE').setValue(pac);
        this.ordenesForm.get('DATOS').setValue(datos1);
        this.ordenesForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.ordenesForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.ordenesForm.get('ID_CITA').setValue(this.id_cita);
        this.ordenesForm.get('SERVICIO').setValue(this.servicio);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.ordenesForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.ordenesForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    GenerarPdf(id) {
        ...
        this.historiasService.getHistoriaPacientes(id)
            .subscribe(data => {
                ...
                html += this.Page1(da.campos, da) + this.Prestador(da.usuario);
                $(div).prop('id', 'print'+1);
    ...
    Page1(datos, da) {
        ...
        var html = '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;">DIAGNOSTICOS:</div>' +
                ...
                html += '</div><div class="row mb-5" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-1" style="background-color: #f3f3f3;font-weight: bolder;">No:</div>' +
                '<div class="col-md-1" style="background-color: #f3f3f3;font-weight: bolder;">Código</div>' +
                '<div class="col-md-5" style="background-color: #f3f3f3;font-weight: bolder;">Descripción</div>' +
                '<div class="col-md-2" style="background-color: #f3f3f3;font-weight: bolder;">Cantidad</div>' +
                '<div class="col-md-3" style="background-color: #f3f3f3;font-weight: bolder;">Observaciones</div>' +
                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>';
                for(var i in serv) {
                	let pos = parseInt(i) + 1;
                	html += '<div class="col-md-1">'+pos+'</div>' +
		                '<div class="col-md-1">'+serv[i]['items']['COD_ITEM']+'</div>' +
		                '<div class="col-md-5">'+serv[i]['items']['NOM_ITEM']+'</div>' +
		                '<div class="col-md-2">'+serv[i]['CANTIDAD']+'</div>' +
		                '<div class="col-md-3">'+serv[i]['DESCRIPCION']+'</div>' +
		                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>';
                }
                html += '</div>';
        return html;
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>HISTORIA CLINICA ORDENES MEDICAS</h3></div>';
        return html;
    }
    ...
    clearAll() {
        this.ordenesForm.get('DATOSHISTORIA').setValue('');
        this.ordenesForm.get('DATOSPACIENTE').setValue('');
        this.ordenesForm.get('DATOS').setValue('');
        this.ordenesForm.get('PACIENTE').setValue('');
        this.ordenesForm.get('PRESTADOR').setValue('');
        this.ordenesForm.get('ID_HISTORIA').setValue(0);
        this.ordenesForm.get('ID_CITA').setValue(0);
    ...
    Solicitar() {
    	...
    	if($('#DESCRIPCION').val() == '') {
    		alert("Por favor, diga la descripción del servicio.");
    		return false;
    	}
    	this.servicio.push({'servicio': $('#CODIGO option:selected').html(), 'cantidad': $('#CANTIDAD').val(), 'descripcion': $('#DESCRIPCION').val(),  'id': $('#CODIGO option:selected').val()});
		let html = $('#addexamen').html();
		let pos = this.servicio.length - 1;
		html = html + '<div class="col-md-5 mb-1 mt-1">'+$('#CODIGO option:selected').html()+'</div>'+
				'<div class="col-md-2 mb-1 mt-1 text-center">'+$('#CANTIDAD').val()+'</div>'+
				'<div class="col-md-3 mb-1 mt-1">'+$('#DESCRIPCION').val()+'</div>'+
				'<div class="col-md-2 mb-1 mt-2"><a href="#" pos="'+pos+'" title="Eliminar servicio" data-toggle="tooltip" class="boton_examen"><i class="zmdi zmdi-delete"></i></a></div>';
		$('#addexamen').html(html);
		$('#CODIGO').val('').trigger('change');
		$('#CANTIDAD').val(1);
		$('#DESCRIPCION').val('No refiere');
    }
