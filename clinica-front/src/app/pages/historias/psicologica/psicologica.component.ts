...
export class PsicologicaComponent implements OnInit {
    ...
    posf: any = -1;
  	...
  	ngOnChanges(changes: SimpleChanges) {
        ...
        const fm: SimpleChange = changes.fami;
        ...
        if(tipo)
            if(tipo.currentValue == 30 && pr) {
                if(pr.currentValue == true)
                    this.GenerarPdf(this.historia_id);
            }
        if(fm)
            if(fm.currentValue.length > 0) {
                $('#addperson').html('');
	            this.familiares = fm.currentValue;
	            var html = "";
	            for(var i in this.familiares){
	                let html = $('#addperson').html();
	                html = html + '<div class="col-md-4 mb-1 mt-1">'+this.familiares[i]['nombre']+'</div>'+
								'<div class="col-md-1 mb-1 mt-1" style="margin-left: 11px;">'+this.familiares[i]['edad']+'</div>'+
								'<div class="col-md-3 mb-1 mt-1" style="margin-left: 0;">'+this.familiares[i]['parent']+'</div>'+
								'<div class="col-md-3 mb-1 mt-1" style="margin-left: 10px;">'+this.familiares[i]['ocup']+'</div>'+
								'<div class="col-md-1 mb-1 mt-2" style="margin-left: -40px;"><a href="#" pos="'+i+'" title="Eliminar familiar" data-toggle="tooltip" class="boton_f"><i class="zmdi zmdi-delete"></i></a> '+
                                '<a href="#" pos="'+i+'" title="Editar familiar" data-toggle="tooltip" class="boton_ed"><i class="zmdi zmdi-edit"></i></a></div>';
	                $('#addperson').html(html);
	            }
            }
            else
	...
	ngAfterViewInit(): void {
        ...
        $("body").on("click", ".boton_f", function(a) {
            a.preventDefault();
            let arr = [];
            let pos = 0;
            for(var i in that.familiares) {
                if(i != $(this).attr('pos')) {
                    arr[pos] = that.familiares[i];
                    pos++;
                }
            }
            $('#addperson').html('');
            ...
            for(var i in that.familiares){
                let html = $('#addperson').html();
                html = html + '<div class="col-md-4 mb-1 mt-1">'+arr[i]['nombre']+'</div>'+
							'<div class="col-md-1 mb-1 mt-1" style="margin-left: 11px;">'+arr[i]['edad']+'</div>'+
							'<div class="col-md-3 mb-1 mt-1" style="margin-left: 0;">'+arr[i]['parentesco']+'</div>'+
							'<div class="col-md-3 mb-1 mt-1" style="margin-left: 10px;">'+arr[i]['ocupacion']+'</div>'+
							'<div class="col-md-1 mb-1 mt-2" style="margin-left: -40px;"><a href="#" pos="'+i+'" title="Eliminar familiar" data-toggle="tooltip" class="boton_f"><i class="zmdi zmdi-delete"></i></a> '+
                            '<a href="#" pos="'+i+'" title="Editar familiar" data-toggle="tooltip" class="boton_ed"><i class="zmdi zmdi-edit"></i></a></div>';
                $('#addperson').html(html);
            }
        ...
        $("body").on("click", ".boton_ed", function(a) {
            a.preventDefault();
            let arr = [];
            let pos = 0;
            for(var i in that.familiares) {
                if(i == $(this).attr('pos')) {
                    $('#nombres').val(that.familiares[i].nombre);
                    $('#edades').val(that.familiares[i].edad);
                    $('#PARENTESCOS').val(that.familiares[i].parentesco).trigger('change');
                    $('#OCUPACIONES').val(that.familiares[i].ocupacion).trigger('change');
                    that.update = true;
                    that.posf = i;
                }
            }
  	...
  	initForm() {
        this.psicologicaForm = this.formBuilder.group({
            DATOSHISTORIA: [''],
            DATOSPACIENTE: [''],
            ID_HISTORIA: [''],
            DATOS: [''],
    ...
    Asignarf() {
        let that: any = this;
    	if($('#nombres').val() == '') {
      		alert('Por favor, diga el nombre del familiar');
      		return false;
    	}
    	else
    	if($('#edades').val() == '') {
      		alert('Por favor, diga la edad del familiar');
      		return false;
    	}

        if(this.update == false) {
    		this.familiares.push({'nombre': $('#nombres').val(), 'edad': $('#edades').val(), 'parentesco': $('#PARENTESCOS').val(), 'ocupacion': $('#OCUPACIONES').val()});
    		let html = $('#addperson').html();
    		let pos = this.familiares.length - 1;
            $('[data-toggle="tooltip"]').tooltip();
    		html = html + '<div class="row col-md-12"><div class="col-md-4 mb-1 mt-1">'+$('#nombres').val()+'</div>'+
    			'<div class="col-md-1 mb-1 mt-1" style="margin-left: 11px;">'+$('#edades').val()+'</div>'+
    			'<div class="col-md-3 mb-1 mt-1" style="margin-left: 0;">'+$('#PARENTESCOS option:selected').html()+'</div>'+
    			'<div class="col-md-3 mb-1 mt-1" style="margin-left: 10px;">'+$('#OCUPACIONES option:selected').html()+'</div>'+
    			'<div class="col-md-1 mb-1 mt-2" style="margin-left: -40px;"><a href="#" pos="'+pos+'" title="Eliminar familiar" data-toggle="tooltip" class="boton_f"><i class="zmdi zmdi-delete"></i></a> '+
                '<a href="#" pos="'+pos+'" title="Editar familiar" data-toggle="tooltip" class="boton_ed"><i class="zmdi zmdi-edit"></i></a></div></div>';
    		$('#addperson').html(html);
        }
        else {
            this.familiares.splice(this.posf, 1, {'nombre': $('#nombres').val(), 'edad': $('#edades').val(), 'parentesco': $('#PARENTESCOS').val(), 'ocupacion': $('#OCUPACIONES').val()});
            let child: any = $('#addperson').children();
            $(child).each(function(i, value) {
                if(that.posf == i) {
                    $(value).children('div[class="col-md-4 mb-1 mt-1"]').html($('#nombres').val());
                    $(value).children('div[style="margin-left: 11px;"]').html($('#edades').val());
                    $(value).children('div[style="margin-left: 0;"]').html($('#PARENTESCOS option:selected').html());
                    $(value).children('div[style="margin-left: 10px;"]').html($('#OCUPACIONES option:selected').html());
                }
            });
            this.update = false;
            this.posf = -1;
		...
		$('#nombres').val('');
		$('#edades').val('');
		$('#PARENTESCOS').val(23).trigger('changes');
		$('#OCUPACIONES').val(511).trigger('changes');
    }
    ...
    Guardar() {
        ...
        this.psicologicaForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.psicologicaForm.get('DATOSHISTORIA').setValue(datos);
    	this.psicologicaForm.get('DATOSPACIENTE').setValue(pac);
        this.psicologicaForm.get('DATOS').setValue(datos1);
        this.psicologicaForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.psicologicaForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.psicologicaForm.get('ID_CITA').setValue(this.id_cita);
        this.psicologicaForm.get('FAMILIAS').setValue(this.familiares);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.psicologicaForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.psicologicaForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    GenerarPdf(id) {
        ...
        this.historiasService.getHistoriaPacientes(id)
            .subscribe(data => {
                ...
                html = header + this.Page3(da.campos,da,da.familiares) + this.Prestador(da.usuario);
    ...
    Page1(datos, page = null) {
        let motivo = datos[58]['VALOR'] != null ? datos[58]['VALOR'] : "NO REFIERE";
        let enfermedad = datos[59]['VALOR'] != null ? datos[59]['VALOR'] : "NO REFIERE";
        var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;padding-right: 15px;padding-left: 15px;flex: 0 0 99%;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">DESENCADENANTE DEL PROBLEMA:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;height: 60px;">'+enfermedad+'</div>' +
                '</div>' +
        		'<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">MANTENEDOR DEL PROBLEMA:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;height: 60px;">'+(datos[60]['VALOR'] != null ? datos[60]['VALOR'] : "NO REFIERE")+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">ENTREVISTA:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA ESCOLAR:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[6]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA FAMILIAR:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[7]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA PAREJA:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[8]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA SEXUAL:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[9]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA SOCIAL:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[10]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA LABORAL:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[11]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA RELIGION:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[12]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA PERSONAL:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[13]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">AREA ECONOMICA:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[14]['VALOR']+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">VIOLENCIA FAMILIAR:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">Alguna vez ha sido maltratado física y/o emocionalmente por alguien?:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[15]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">Se siente en riesgo en este momento?:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[16]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">Tipo de Violencia:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[17]['VALOR']+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">IDEACION SUICIDA Y ACTITUDES ANTE LA VIDA:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">CONDUCTA SUICIDA:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[18]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">RED DE APOYO:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[19]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">REMISION:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[20]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">CONSUMO DE SPA:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[21]['VALOR']+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">INFORMACION Y EDUCACION:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">ORIENTACION GENERAL:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[22]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">VIOLENCIA Y MALTRATO:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[23]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">RUTAS DE VIOLENCIA:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[24]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">GESTANTE - CASO IVE:</div>' +
    ...
    Page2(datos, page = null) {
        var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">ANTECEDENTES DEL MENOR (entre 0 y 13 años):</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">ANTECEDENTES DE LA MADRE EN EL PERIODO DE GESTACION:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[38]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">COMPLICACIONES EN EL EMBARAZO:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[39]['VALOR']+'</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">TIPOD E PARTO:</div>' +
                '<div class="col-md-6" style="text-transform: uppercase !important;">'+datos[40]['VALOR']+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">COMO RESUELVE LOS PROBLEMAS CON SU HIJO/TECNICAS DISCIPLINARIAS:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;height: 100px;">'+(datos[61]['VALOR'] != null ? datos[61]['VALOR'] : "NO REFIERE")+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">ANAMNESIS:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(datos[62]['VALOR'] != null ? datos[62]['VALOR'] : "NO REFIERE")+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">ESPECTATIVAS FRENTE AL PROCESO DE INTERVENCION:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(datos[63]['VALOR'] != null ? datos[63]['VALOR'] : "NO REFIERE")+'</div>' +
                '</div>';
        return html;
    }

    Page3(datos, da, familiares, page = null) {
        var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(datos[64]['VALOR'] != null ? datos[64]['VALOR'] : "NO REFIERE")+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(da.DIAGNOSTICO != '' ? da.DIAGNOSTICO : "NO REFIERE")+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">PLAN DE MANEJO:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>' +
                '<div class="col-md-12" style="text-transform: uppercase !important;">'+(datos[65]['VALOR'] != null ? datos[65]['VALOR'] : "NO REFIERE")+'</div>' +
                '</div>' +
                '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;">ESTRUCTURA FAMILIAR:</div>' +
                '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div>';
        if(familiares.length > 0) {
            html += '<div class="row col-md-12" style="font-size: 13px;vertical-align: top !important;display: block;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-right: 15px;box-sizing: content-box;overflow: visible;">';
            for(var i in familiares) {
                html += '<div class="row col-md-12" style="-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-right: 15px;box-sizing: content-box;overflow: visible;">' +
                    '<div class="col-md-2" style="text-transform: uppercase !important;">nombre y apellido:</div>' +
                    '<div class="col-md-10" style="text-transform: uppercase !important;">'+familiares[i]['NOMBRE']+'</div>' +
                    '<div class="col-md-2" style="text-transform: uppercase !important;">edad:</div>' +
                    '<div class="col-md-10" style="text-transform: uppercase !important;">'+familiares[i]['EDAD']+'</div>' +
                    '<div class="col-md-2" style="text-transform: uppercase !important;">parentesco:</div>' +
                    '<div class="col-md-10" style="text-transform: uppercase !important;">'+familiares[i]['parentesco']['NOM_PARENTESCO']+'</div>' +
                    '<div class="col-md-2" style="text-transform: uppercase !important;">ocupación:</div>' +
                    '<div class="col-md-10" style="text-transform: uppercase !important;">'+familiares[i]['ocupacion']['NOM_OCUPACION']+'</div>' +
                    '<div class="col-md-12" style="height: 0;border: 0; border-top: 1px solid #aaaaaa;-webkit-box-flex: 0;-ms-flex: 0 0 100%;flex: 0 0 100%;margin-left: -15px;box-sizing: content-box;overflow: visible;"></div></div>';
            }
            html += '</div>';
        }
        else {
            html += '<div class="col-md-12" style="text-transform: uppercase !important;">NO REFIERE</div>';
        }
        html += '</div>';
        return html;
    }
    ...
    OtrosDatos(da) {
        ...
        var html = '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 15px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>HISTORIA CLINICA PSICOLOGICA</h3></div>';
        return html;
    }
    ...
    CrearImagen(pdf, div, terminar) {
        ...
        html2canvas(div).then(canvas => {                           
            ...
            if(terminar == true) {
                ...
                that.tipo_id.emit(0);
                /*pdf.autoPrint();
                pdf.save('HISTORIACLINICAPSICOLOGICA.pdf');*/
    ...
    clearAll() {
        this.psicologicaForm.get('DATOSHISTORIA').setValue('');
        this.psicologicaForm.get('DATOSPACIENTE').setValue('');
        this.psicologicaForm.get('DATOS').setValue('');
        this.psicologicaForm.get('PACIENTE').setValue('');
        this.psicologicaForm.get('PRESTADOR').setValue('');
        this.psicologicaForm.get('ID_HISTORIA').setValue(0);
        this.psicologicaForm.get('ID_CITA').setValue(0);
