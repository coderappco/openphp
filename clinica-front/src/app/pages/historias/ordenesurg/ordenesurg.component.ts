...
@Component({
  	selector: 'app-ordenesurg',
  	templateUrl: './ordenesurg.component.html',
  	styleUrls: ['./ordenesurg.component.css']
})
export class OrdenesurgComponent implements OnInit {
    ...
    @Input() exam: any;
  	...
  	ngOnChanges(changes: SimpleChanges) {
        ...
        const ex: SimpleChange = changes.exam;
        ...
        if(tipo)
            if(tipo.currentValue == 22 && pr) {
                if(pr.currentValue == true)
                    this.GenerarPdf(this.historia_id);
            }
        if(ex)
            if(ex.currentValue.length > 0) {
                $('#addexamen').html('');
	            this.examenes = ex.currentValue;
	            var html = "";
	            for(var i in this.examenes){
	                let html = $('#addexamen').html();
	                html = html + '<div class="col-md-7 mb-1 mt-1">'+this.examenes[i]['examen']+'</div>'+
								'<div class="col-md-3 mb-1 mt-1 text-center">'+this.examenes[i]['cantidad']+'</div>'+
								'<div class="col-md-2 mb-1 mt-2"><a href="#" pos="'+i+'" title="Eliminar examen" data-toggle="tooltip" class="boton_examen"><i class="zmdi zmdi-delete"></i></a></div>';
	                $('#addexamen').html(html);
	            }
            }
            else
	...
	ngAfterViewInit(): void {
        ...
        $("body").on("click", ".boton_examen", function(a) {
            ...
            let arr = [];
            let pos = 0;
            for(var i in that.examenes) {
                if(i != $(this).attr('pos')) {
                    arr[pos] = that.examenes[i];
                    pos++;
                }
            }
            $('#addexamen').html('');
            ...
            for(var i in that.examenes){
                let html = $('#addexamen').html();
                html = html + '<div class="col-md-7 mb-1 mt-1">'+arr[i]['examen']+'</div>'+
                              '<div class="col-md-3 mb-1 mt-1 text-center">'+arr[i]['cantidad']+'</div>'+
                              '<div class="col-md-2 mb-1 mt-2"><a href="#" pos="'+i+'" title="Eliminar examen" data-toggle="tooltip" class="boton_examen"><i class="zmdi zmdi-delete"></i></a></div>';
                $('#addexamen').html(html);
            }
        });
    ...
    Guardar() {
    	...
    	if(this.examenes.length == 0) {
    		alert("Por favor, debe asignar los examenes al paciente.");
    		return false;
    	}
        ...
        this.ordenesForm.get('EXAMENES').setValue(this.examenes);
    ...
    Page1(datos, da) {
    	var exm = da.examenurg;
        var html = '<div class="col-md-6">Número de orden: '+da.ID_HISTORIA_PACIENTE+'</div>' +
        		'<div class="col-md-6">Fecha Diligenciada: '+moment(da.FEC_DILIGENCIADA).format('YYYY-MM-DD H:mm')+'</div>' +
        		'<div class="row mb-5" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-1" style="background-color: #f3f3f3;font-weight: bolder;">No:</div>' +
                '<div class="col-md-3" style="background-color: #f3f3f3;font-weight: bolder;">Código</div>' +
                '<div class="col-md-6" style="background-color: #f3f3f3;font-weight: bolder;">Descripción</div>' +
                ...
                for(var i in exm) {
                	...
                	html += '<div class="col-md-1">'+pos+'</div>' +
		                '<div class="col-md-3">'+exm[i]['items']['COD_ITEM']+'</div>' +
		                '<div class="col-md-6">'+exm[i]['items']['NOM_ITEM']+'</div>' +
		                '<div class="col-md-2">'+exm[i]['CANTIDAD']+'</div>' +
		                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>';
                }
                html += '</div>';
        return html;
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>RELACION GENERAL DE ORDENES MEDICAS</h3></div>';
        return html;
    }
    ...
    Solicitar() {
    	...
    	this.examenes.push({'examen': $('#CODIGO option:selected').html(), 'cantidad': $('#CANTIDAD').val(), 'id': $('#CODIGO option:selected').val()});
		let html = $('#addexamen').html();
		let pos = this.examenes.length - 1;
		html = html + '<div class="col-md-7 mb-1 mt-1">'+$('#CODIGO option:selected').html()+'</div>'+
				'<div class="col-md-3 mb-1 mt-1 text-center">'+$('#CANTIDAD').val()+'</div>'+
				'<div class="col-md-2 mb-1 mt-2"><a href="#" pos="'+pos+'" title="Eliminar examen" data-toggle="tooltip" class="boton_examen"><i class="zmdi zmdi-delete"></i></a></div>';
		$('#addexamen').html(html);
