...
export class BasicaComponent implements OnInit {
    ...
    id_cita: any = 0;
  	...
  	constructor(private formBuilder: FormBuilder, private codifService: CodifService, private globals: Globals, private historiasService: HistoriasService,
  				private _loadingBar: SlimLoadingBarService, private router: Router) { }
  	...
  	initForm() {
        this.basicaForm = this.formBuilder.group({
            DATOSHISTORIA: [''],
            DATOSPACIENTE: [''],
            ID_HISTORIA: [''],
            DATOS: [''],
    ...
    Guardar() {
        ...
        this.basicaForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
        this.basicaForm.get('DATOSHISTORIA').setValue(datos);
        this.basicaForm.get('DATOSPACIENTE').setValue(pac);
        this.basicaForm.get('DATOS').setValue(datos1);
        this.basicaForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.basicaForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.basicaForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
            ...
            this.historiasService.saveHistoriaPaciente(this.basicaForm.value)
                .subscribe(data => {
                    let da: any = data;
                    this.clearAll();
            ...
            this.historiasService.updateHistoriaPaciente(this.historia_id, this.basicaForm.value)
                .subscribe(data => {
                    let da: any = data;
                    this.clearAll();
    ...
    clearAll() {
        this.basicaForm.get('DATOSHISTORIA').setValue('');
        this.basicaForm.get('DATOSPACIENTE').setValue('');
        this.basicaForm.get('DATOS').setValue('');
        this.basicaForm.get('PACIENTE').setValue('');
        this.basicaForm.get('PRESTADOR').setValue('');
        this.basicaForm.get('ID_HISTORIA').setValue(0);
        this.basicaForm.get('ID_CITA').setValue(0);
    ...
    GenerarPdf(id) {
        ...
        this.historiasService.getHistoriaPacientes(id)
            .subscribe(data => {
				...
				html += this.Prestador(da.usuario);
				$(div).prop('id', 'print'+1);
	...
	Prestador(usuario, page = null) {
		...
		var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 18px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
	...
	OtrosDatos(da) {
		...
		let motivo = da.campos[4]['VALOR'] != null ? da.campos[4]['VALOR'] : "-";
		var html = '<div class="row" style="margin-bottom: 2rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;padding-right: 15px;padding-left: 15px;font-size: 18px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                ...
                '<div class="text-left" style="width:17%; margin: 0;padding: 0;text-transform: uppercase !important;">'+da.paciente.religion.NOM_RELIGION+'</div>' +
    			...
    			'</div><div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;"><h3>MOTIVO DE CONSULTA:</h3></div>' +
                '<div style="width:100%;box-sizing: content-box;height: 0;overflow: visible; border: 0; border-top: 1px solid #aaaaaa;"></div>' +
    			'<div style="width:99%;margin-bottom: 2rem !important;">'+motivo+'</div>';
    	return html;
	}
	...
	Header(empresa, paciente, id) {
    	...
    	var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
    			...
    			'<div class="col-md-12 text-center"><h3>HISTORIA CLINICA BASICA</h3></div>';
    	return html;
    }
    ...
    CrearImagen(pdf, div, terminar) {
        ...
        html2canvas(div).then(canvas => {
            ...
            if(terminar == true) {
                ...
                that.tipo_id.emit(0);
                /*pdf.autoPrint();
                pdf.save('Historia_clinica_basica.pdf');*/
