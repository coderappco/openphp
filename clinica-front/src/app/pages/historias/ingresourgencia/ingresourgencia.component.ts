...
@Component({
  	selector: 'app-ingresourgencia',
  	templateUrl: './ingresourgencia.component.html',
  	styleUrls: ['./ingresourgencia.component.css']
})
export class IngresourgenciaComponent implements OnInit {

	ingresouForm: FormGroup;
	escolaridad: any = [];
    etnia: any = [];
    ocupacion: any = [];
    discapacidad: any = [];
	...
	ngAfterViewInit(): void {
  	...
  	initForm() {
        this.ingresouForm = this.formBuilder.group({
            DATOSHISTORIA: [''],
            DATOSPACIENTE: [''],
            ID_HISTORIA: [''],
            DATOS: [''],
    ...
    Guardar() {
        ...
        this.ingresouForm.get('ID_HISTORIA').setValue($('#HISTORIA').val());
    	this.ingresouForm.get('DATOSHISTORIA').setValue(datos);
    	this.ingresouForm.get('DATOSPACIENTE').setValue(pac);
        this.ingresouForm.get('DATOS').setValue(datos1);
        this.ingresouForm.get('PACIENTE').setValue($('#PACIENTE').val());
        this.ingresouForm.get('PRESTADOR').setValue(this.role == 'ADMINISTRADOR' ? $('#ID_USUARIO').val() : this.id_user);
        this.ingresouForm.get('ID_CITA').setValue(this.id_cita);
        if(this.historia_id == 0) {
	        ...
	        this.historiasService.saveHistoriaPaciente(this.ingresouForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
	    	...
	    	this.historiasService.updateHistoriaPaciente(this.historia_id, this.ingresouForm.value)
	        	.subscribe(data => {
	        		let da: any = data;
	        		this.clearAll();
    ...
    Page1(datos, da) {
        ...
        var html = '<div class="row" style="margin-bottom: 1rem !important;-webkit-box-flex: 0;-ms-flex: 0 0 99%;flex: 0 0 99%;font-size: 12px; box-sizing: content-box;overflow: visible; border: 0; border: 1px solid #e9ecef;">' +
                '<div class="col-md-12" style="background-color: #f3f3f3;font-weight: bolder;">DATOS GENERALES:</div>' +
    ...
    Header(empresa, paciente, id) {
        ...
        var html = '<div class="col-md-3 text-left" style="margin: 0;padding: 0;font-size: 12px;">Fecha Impresión: '+fechai+'</div>' +
                ...
                '<div class="col-md-12 text-center"><h3>HISTORIA CLINICA INGRESO URGENCIA</h3></div>';
        return html;
    }
    ...
    clearAll() {
        this.ingresouForm.get('DATOSHISTORIA').setValue('');
        this.ingresouForm.get('DATOSPACIENTE').setValue('');
        this.ingresouForm.get('DATOS').setValue('');
        this.ingresouForm.get('PACIENTE').setValue('');
        this.ingresouForm.get('PRESTADOR').setValue('');
        this.ingresouForm.get('ID_HISTORIA').setValue(0);
        this.ingresouForm.get('ID_CITA').setValue(0);
