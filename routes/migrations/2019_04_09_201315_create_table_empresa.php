...
class CreateTableEmpresa extends Migration
{
    ...
    public function up()
    {
        Schema::create('CFG_EMPRESA', function (Blueprint $table) {
            ...
            $table->string('NUM_DOC_REP_LEGAL'); //NUMERO DE CEDULA DEL REPRESENTANTE LEGAL DE LA EMPRESA
            $table->string('NOM_REP_LEGAL'); //NOMBRE DEL GERENTE O REPRESENTANTE LEGAL
            $table->string('DIREC_EMP')->nullable(); //DIRECCION DE LA EMPRESA
            $table->string('TELEF')->nullable(); //TELEFONO 1 DE LA EMPRESA
            $table->string('CORREO'); //CORREO ELECTRONICO DE LA EMPRESA
